...
import { getTypeOfUser } from './../../store/general.store';
// import { getMenuOptions } from '@router/routes.config';
import { produce } from 'immer';
import { getMenuOptions } from 'config/routes.config';
...
export const useNavigationOptions = () => {
    ...
    console.log('routerTree', routeTree)

    // set menu options
    useEffect(() => {
        const defaultMenuOptions = getMenuOptions()

        // const optionsToShow = defaultMenuOptions.filter((option: any) => option.permission.includes(typeOfUser!))
        const optionsToShow = defaultMenuOptions
